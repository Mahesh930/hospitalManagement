...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    /**
     * Bypasses filter execution for public authentication endpoints (/auth/login, /auth/signup, /oauth2/**).
     */
    @Override
    protected boolean shouldNotFilter(HttpServletRequest request) {
        String path = request.getRequestURI();
        return path.contains("/auth/login") || path.contains("/auth/signup") || path.contains("/oauth2/");
    }
